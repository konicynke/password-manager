...
def add_new_vault_item(data, request, db):
...
def get_vault_items(request, db):
    current_user_id = request.session.get('user_id')
    user_master_key = active_key.get(current_user_id)
    items_list = []
    ...
    vault_items = db.query(VaultItem).filter_by(user_id=current_user_id).all()
...
def delete_vault_item(item_id, request, db):
    current_user_id = request.session.get('user_id')
    ...
    try:
        deleted_count = db.query(VaultItem).filter_by(user_id=current_user_id, id=item_id).delete()
        if deleted_count == 0:
...
def update_vault_item(item_id, data, request, db):
    current_user_id = request.session.get('user_id')
    user_master_key = active_key.get(current_user_id)
    ...
    vault_item = db.query(VaultItem).filter_by(user_id=current_user_id, id=item_id).first()
    if not vault_item:
...
def audit_passwords(request, db):
    current_user_id = request.session.get('user_id')
    user_master_key = active_key.get(current_user_id)
    ...
    vault_items = get_vault_items(request, db)
